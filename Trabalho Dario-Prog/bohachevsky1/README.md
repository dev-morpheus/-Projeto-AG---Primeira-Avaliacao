# Trabalho avaliativo - Algoritmo Genético Função Bohachevsky 1 (BC1)
## Aluno: Vitor dos Santos Correia
...
### BF1: f(x) = x1² + 2x2² - 0.3 * cos(3 * pi * x1) - 0.4 * cos(4 * pi * x2) + 0.7
<br>

### Tabela Comparativa

| BF1 | Ali 2010 | Kaela 2007 | Vitor 2026 |
| :--- | :---: | :---: | :---: |
| NFE | 2.743 | 1.252 | 85.874 |
| SR | 100 | 100 | 100 |

- ### Fitness: Representação real direta da função de Bohachevsky 1. O ótimo global é 0 na origem.
- ### Crossover: Aritmético com taxa de 90%, utilizando $\alpha=0.8$ para dar preferência à carga genética do pai mais apto (elitismo local).
- ### Mutação: Mutação Gaussiana com Decaimento Linear. Em vez de substituição aleatória, foi adicionado um ruído cuja amplitude (escala) diminui conforme as gerações avançam. Isso permitiu que o AG escapasse de mínimos locais no início e fizesse um ajuste de precisão no final da busca.
- ### Seleção: Torneio binário, para a escolha dos pais.
<br>
...
def bf1_objective(x):
    x = np.atleast_2d(x)
    x1, x2 = x[:, 0], x[:, 1]
    # f(x) = x1² + 2x2² - 0.3*cos(3*pi*x1) - 0.4*cos(4*pi*x2) + 0.7
    return x1**2 + 2*x2**2 - 0.3*np.cos(3*np.pi*x1) - 0.4*np.cos(4*np.pi*x2) + 0.7

def executar_ag_bf1_corrigido():
    lb, ub = -50.0, 50.0 
    size_pop = 100 
    ...
    for geracao in range(num_geracoes_max):
        ...
        # Critério de parada: Estabilidade ou atingir o ótimo
...
if __name__ == "__main__":
    ...
    print("Executando 100 repetições para BF1.")

    for r in range(num_repeticoes):
        res, nfe_run = executar_ag_bf1_corrigido()
        total_nfe += nfe_run
        if abs(res - 0.0) < 0.01: 
            sucessos += 1
